# revoice common functions
# published under GNU General Public License v3.0
# Tuku <tuku@tuxzz.org>, Otonashi Hikaria <hikaria@tuxzz.org>
...
def loadWav(path):
    ...
    if(w.dtype == np.uint8):
        w = w.astype(np.float32) / 127.5 - 1.0
...
def calcSpectrumAtFreq(x, freqList, sr):
    ...
    t = np.arange(-nX // 2, nX - nX // 2) / sr
    o = np.zeros(nFreq, dtype = np.complex64)
    n2jpit = -2j * np.pi * t
    for i in range(freqList.shape[0]):
        freq = freqList[i]
...
def applySmoothingFilter(x, order):
    ...
    for i in range(halfOrder, nX - halfOrder):
        ...
        dTemp = x[lowerIdx:upperIdx] - mean
        dTemp[dTemp < 0.0] = 0.0
        dTotal = np.sum(dTemp)
...
@nb.njit()
def ensureDivide(x: int, n: int):
    return x + (n - x % n) if x % n != 0 else x

@nb.njit(nb.complex64[:](nb.float32[:], nb.float32[:], nb.float32, nb.float32), fastmath=True, parallel=True, cache=True)
def fcht(x, freqList, a, sr):
    nX = x.shape[0]
    nFreq = freqList.shape[0]
    a /= sr
    max_a = sr / max(nX // 2, nX - nX // 2)
    a = max(-max_a, min(a, max_a))
    
    t = np.arange(-nX // 2, nX - nX // 2) / sr
    o = np.zeros(nFreq, dtype = np.complex64)
    n2jpit = -2j * np.pi * t * (1.0 + a * t)
    for i in nb.prange(freqList.shape[0]):
        v = np.sum(x * np.exp(n2jpit * freqList[i]))
        o[i] = v

    return o

def chirp_rate_from_f0_list(f0_list, hop_size, sr, smooth):
    hop_t = hop_size / sr
    n = 0
    a_list = np.zeros_like(f0_list, dtype=np.float32)
    for seg in splitArray(f0_list):
        seg_len = len(seg)
        if seg[0] > 0.0:
            if seg_len > 1:
                if smooth:
                    seg = applySmoothingFilter(seg, 3)
                a_list[n] = (seg[1] - seg[0]) / hop_t
            for i in range(1, seg_len - 1):
                a_list[n + i] = ((seg[i] - seg[i - 1]) + (seg[i + 1] - seg[i])) * 0.5 / hop_t
            if seg_len > 1:
                a_list[n + seg_len - 1] = (seg[seg_len - 1] - seg[seg_len - 2]) / hop_t
        n += seg_len
    return a_list