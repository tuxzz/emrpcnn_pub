...
def eval_gene_core(gene):
  ...
  with graph.as_default():
    ...
    with tf.compat.v1.Session(config=sess_conf) as sess:
      # TRAIN
      p_feature = tf.compat.v1.placeholder(tf.float32, shape=(cfg.batch_size, cfg.n_hop_per_sample, n_feature, 1))
      p_target = tf.compat.v1.placeholder(tf.float32, shape=(cfg.batch_size, cfg.n_hop_per_sample, n_feature, n_out_channel))
      ...
      valid_sdr_list = []
      ret_list = []
      for (real_vocal, real_inst, magn_orig_list, phase_list, norm_fac) in eval_seg_list:
...
def init_data():
  ...
  random.seed(0x41526941)
  np.random.seed(0x41526941)
  for (root, dir_list, file_list) in os.walk(cfg.mir_wav_path, followlinks=True):
    for i_file, filename in enumerate(np.random.permutation([x for x in sorted(file_list) if x.startswith("abjones") or x.startswith("amy")])):
      ...
      print("  %04d:%s" % (i_file, filename,))
      path = os.path.join(root, filename)
      w, sr = loadWav(path)
      w_mixed = np.sum(w, axis=1)
      w_vocal = w[:, 1]
      w_inst = w[:, 0]
