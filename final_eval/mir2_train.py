...
with cfg.ConfigBoundary():
  # Load data
  batch_size = cfg.MIR2Config.batch_size
  cache_path = cfg.mir2_cache_path
  ...
  v_lr = p_lr_fac * tf.compat.v1.train.cosine_decay_restarts(cfg.MIR2Config.max_lr, v_global_step, cfg.MIR2Config.first_decay_period, alpha=cfg.MIR2Config.min_lr / cfg.MIR2Config.max_lr, t_mul=2.0)
  ...
  with tf.compat.v1.Session(config=cfg.sess_cfg) as sess:
    ...
    ckpt = tf.train.get_checkpoint_state(cfg.MIR2Config.ckpt_path)
    if ckpt and ckpt.model_checkpoint_path:
      print("* Load checkpoint")
      tf.compat.v1.train.Saver().restore(sess, ckpt.model_checkpoint_path)
    
    t = time.time()
    for step in range(v_global_step.eval(), cfg.MIR2Config.final_step):
      random.seed(step)
      np.random.seed(step)
      ...
      if step % display_interval == 0:
        ...
        print("[%05d] loss=%.9f, d_time=%.3f" % (step, mean_loss, now - t))
        t = time.time()
        loss_list = []

      # Save state
      if step % cfg.MIR2Config.ckpt_step == 0:
        tf.compat.v1.train.Saver().save(sess, os.path.join(cfg.MIR2Config.ckpt_path, "checkpoint"), global_step=step)